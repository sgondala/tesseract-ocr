...
int main(int argc, char **argv) {
  ...
  //return 0;
  //S : The whole filling is being done after this
  if (b) tprintf("%s\n", "S : TXT Output Format"); renderers.push_back(new tesseract::TessTextRenderer(outputbase));
  //return 0;
  if (!renderers.empty()) {
    tprintf("%s\n","S : Done");
    //tprintf("%d\n",renderers.size());
